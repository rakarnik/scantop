...
void get_fasta_fast(istream &test, vector<string>& seqset, vector<string>& nameset, const unsigned int minlen) {
...
void get_fasta_fast(const char* fname, vector<string>& seqset, vector<string>& nameset, const unsigned int minlen) {
	ifstream test(fname);
	if(! test) {
	  cerr << "No such file '" << fname << "'\n";
	  exit(0);
	}
	get_fasta_fast(test, seqset, nameset, minlen);
}

void get_fasta_fast(const char* filename, vector<string>& seqset, const unsigned int minlen){
	//when you don't care about titles
	vector<string> nameset;
	get_fasta_fast(filename, seqset, nameset, minlen);
}
